...
//默认内置方法。
  ...
  var Class = function(setings){
    ...
    that.config.localeConfig = layer.localesConfig[that.config.locale] || layer.localesConfig[layer.defaultLocale];
    that.config['title'] = that.config.localeConfig['title']
...
//创建骨架
  Class.pt.creat = function(){
    ...
    switch(config.type){
      case 0:
        config.btn = ('btn' in config) ? config.btn : locale.btn[0];
    ...
    return layer.open($.extend({
      ...
      ,yes: function(index){
        ...
        if(value === ''){
          ...
          layer.tips(localeConfig.maxLengthError.replace("{}", (options.maxlength || 500)) , prompt, {tips: 1});
        } else {
...
//相册层
  layer.photos = function(options, loop, key){
    ...
    if(!isObject){ //页面直接获取
      ...
      return layer.msg(localeConfig.noImage);
    }
    ...
    loadImage(data[start].src, function(img){
      ...
      layer.close(dict.loadi);
      layer.msg(localeConfig.imageError, {
        time: 30000,
        btn: localeConfig.imageBtn,
        yes: function(){
